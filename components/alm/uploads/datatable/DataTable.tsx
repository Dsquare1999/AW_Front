...
export function DataTable<TData, TValue>({
  ...
  return (
    <div className="rounded-md border">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onAddBondSubmit)}>
                        ...
                        <TableCell key={headerIdx + uuidv4()}>
                          {header.isPlaceholder ? null : header.id !==
                            ...
                            choosenFields.map((bond, bondIndex) => {
                              const SelectedBond =
                                bond as (typeof choosenFields)[0];
                              return SelectedBond.name === header.id ? (
                                SelectedBond.type === "select" &&
                                SelectedBond.options ? (
                                  <div className="relative" key={bondIndex}>
                                    <select
                                      {...form.register(SelectedBond.name)}
                                      name={SelectedBond.name}
                                      id={SelectedBond.name}
